...
printf("The graph of two dice");
	...
	static int triplezar[19];
	int tmp3;
	for (i=0;i<10000;i++){
	     tmp1 = (rand()%6)+1;
	     tmp2 = (rand()%6)+1;
	     tmp3 = (rand()%6)+1;
	    triplezar[tmp1+tmp2+tmp3]++;
	}
...
*/
	...
	//printf("\n\nenter number for second graph scale\n");
	//scanf("%d",&scalefaktor2);
	key=max/scalefaktor3;
/*	for(i=2;i<13;i++)
	printf("temsil:%d\n\n",ciftzar[i]/key);
*/	
	printf("\n\n");
printf("The graph of three dice");
	printf("\n\n\n");
	for(i=scalefaktor3;i>0;i--){
		for(j=3;j<=18;j++){
			
			if((triplezar[j]/key)>=i)
			printf("*");
			else
			printf(" ");
	...
	for(i=3;i<=18;i++){
				printf("%d",i);
				if(i<10) //cift basamakli sayilardaki kaymayi fixle
				printf("  ");
				else
				printf(" ");
} 
